...
definition valid_s :: "'c \<Rightarrow> ('c \<Rightarrow> 'b \<Rightarrow> 'c) \<Rightarrow> ('c \<times> 'b, 'c) mapping \<Rightarrow> ('c \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow>
...
definition valid_window :: "('b, 'c, 'd :: timestamp, 't, 'e) args \<Rightarrow> 't \<Rightarrow> 'e \<Rightarrow> ('d \<times> 'b) list \<Rightarrow>
  ...
  "valid_window args t0 sub rho w \<longleftrightarrow>
    ...
    (\<forall>q bs. case Mapping.lookup ac (q, bs) of None \<Rightarrow> True | Some v \<Rightarrow> accept q bs = v) \<and>
...
lemma valid_init_window: "valid_window args t0 sub [] (init_window args t0 sub)"
  by (auto simp: valid_window_def mmap_keys_def mmap_lookup_def sup_leadsto_def
      ...
      intro: reach_sub.intros split: option.splits)
...
proof (induction m arbitrary: e r m' e')
  ...
  have mmap_fold: "mmap_fold (p # m) f c r = mmap_fold m f c (mmap_combine k v c r)"
    using Cons
    by (auto simp: mmap_fold_def kv_def)
...
definition cac :: "('c \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> ('c \<times> 'b, bool) mapping \<Rightarrow>
  'c \<Rightarrow> 'b \<Rightarrow> (bool \<times> ('c \<times> 'b, bool) mapping)" where
  "cac accept ac q bs = (case Mapping.lookup ac (q, bs) of None \<Rightarrow> (let res = accept q bs in
    (res, Mapping.update (q, bs) res ac)) | Some v \<Rightarrow> (v, ac))"

fun mmap_fold_s :: "('c \<Rightarrow> 'b \<Rightarrow> 'c) \<Rightarrow> ('c \<times> 'b, 'c) mapping \<Rightarrow>
  ('c \<Rightarrow> 'b \<Rightarrow> bool) \<Rightarrow> ('c \<times> 'b, bool) mapping \<Rightarrow>
  'b \<Rightarrow> 'd \<Rightarrow> nat \<Rightarrow> ('c, 'c \<times> ('d \<times> nat) option) mmap \<Rightarrow>
  (('c, 'c \<times> ('d \<times> nat) option) mmap \<times> ('c \<times> 'b, 'c) mapping \<times> ('c \<times> 'b, bool) mapping)" where
  "mmap_fold_s step st accept ac bs t j [] = ([], st, ac)"
| "mmap_fold_s step st accept ac bs t j ((q, (q', tstp)) # qbss) =
    (let (q'', st') = cstep step st q' bs;
         (\<beta>, ac') = cac accept ac q' bs;
...
lemma mmap_fold_s_sound: "mmap_fold_s step st accept ac bs t j qbss = (qbss', st', ac') \<Longrightarrow>
  ...
  (\<And>q bs. case Mapping.lookup ac (q, bs) of None \<Rightarrow> True | Some v \<Rightarrow> accept q bs = v) \<Longrightarrow>
  qbss' = mmap_map (\<lambda>q (q', tstp). (step q' bs, if accept q' bs then Some (t, j) else tstp)) qbss \<and>
  (\<forall>q bs. case Mapping.lookup st' (q, bs) of None \<Rightarrow> True | Some v \<Rightarrow> step q bs = v) \<and>
...
proof (induction qbss arbitrary: st ac qbss')
  ...
  obtain b ac'' where b_def: "cac accept ac q' bs = (b, ac'')"
    "b = accept q' bs"
    using Cons(4)
    by (cases "cac accept ac q' bs")
  ...
  have ih: "\<And>q bs. case Mapping.lookup st'' (q, bs) of None \<Rightarrow> True | Some a \<Rightarrow> step q bs = a"
    "\<And>q bs. case Mapping.lookup ac'' (q, bs) of None \<Rightarrow> True | Some a \<Rightarrow> accept q bs = a"
...
definition adv_end :: "('b, 'c, 'd :: timestamp, 't, 'e) args \<Rightarrow>
  ...
  "adv_end args w = (let step = w_step args; accept = w_accept args;
    ...
    (case run_t tj of Some (tj', t) \<Rightarrow> (case run_sub sj of Some (sj', bs) \<Rightarrow>
      ...
      (e', st'') = mmap_fold' e st'
        (\<lambda>((x, y), st). let (q', st') = cstep step st x bs in ((q', y), st')) sup [] in
      w\<lparr>w_st := st'', w_ac := ac', w_j := Suc j, w_tj := tj', w_sj := sj', w_s := s', w_e := e'\<rparr>)))"
...
proof -
  ...
  have adv_end: "adv_end args w = w\<lparr>w_st := st'', w_ac := ac',
...
definition "loop_body step accept run_t run_sub =
  ...
  case cstep step st' q b of (q', st'') \<Rightarrow> case cac accept ac q b of (\<beta>, ac') \<Rightarrow>
...
proof -
  ...
  proof (rule while_rule_lemma[of "loop_inv init step accept args t0 sub rho i j tj sj"
    ...
    show "loop_inv init step accept args t0 sub rho i j tj sj
      ...
      using i_lt_j valid_s'_Suc_i sup_acc_same[of step accept rho]
        length_rho reach_sub_si' reach_sub_ti' tb_def(3,4) valid_before(4)
  ...
  next
    ...
    obtain q' st''_cur where q'_def: "cstep step st'_cur q_cur b_cur = (q', st''_cur)"
      by fastforce
    obtain \<beta> ac'_cur where b_def: "cac accept ac_cur q_cur b_cur = (\<beta>, ac'_cur)"
      by fastforce
    ...
    have accept: "\<beta> = accept q_cur b_cur" "\<And>q bs. case Mapping.lookup ac'_cur (q, bs) of
      None \<Rightarrow> True | Some v \<Rightarrow> accept q bs = v"
      using assms b_def
    ...
    have q_cur_def: "steps step rho init (i + 1, i_cur) = q_cur"
      ...
      unfolding loop_inv_def state_def
      by auto
    have b_acc: "\<beta> = acc step accept rho init (i + 1, i_cur)"
      unfolding accept(1) acc_def q_cur_def
    ...
    have "reach_window args t0 sub rho (Suc i_cur, ti'_cur, si'_cur, j, tj, sj)"
      using reach_sub_si' reach_sub_ti' tb_cur_def(3,4) length_rho assms(2)
  ...
  have valid_stac_cur: "\<forall>q bs. case Mapping.lookup st_cur (q, bs) of None \<Rightarrow> True
    | Some v \<Rightarrow> step q bs = v" "\<forall>q bs. case Mapping.lookup ac_cur (q, bs) of None \<Rightarrow> True
